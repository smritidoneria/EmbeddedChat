...
// const RoomMembers = ({ fetchMembers }) => {
...
//   const [filteredMembers, setFilteredMembers] = useState(fetchMembers);
...
//     try {
//       await RCInstance.addMember(newMember);
//       fetchMembers(); // Fetch members after adding a new member
//     } catch (error) {
...
//           {showInvite ? (
//             <InviteMembers  onAddMember={handleAddMember}/>
//           ) : (
//             <>
//               {isAdmin && (
//                 <Button
...
const RoomMembers = ({ fetchMembers }) => {
  ...
  const [searchTerm, setSearchTerm] = useState('');
  const members = useMemberStore((state) => state.members) || []; // Initialize members as an empty array
...
RoomMembers.propTypes = {
  fetchMembers: PropTypes.func.isRequired, // Add fetchMembers prop type
};